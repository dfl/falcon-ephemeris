...
# Osculating elements (heliocentric, J2000 ecliptic) at 180-day epochs, 1600-2100, for the
...
for pair in "${IDS[@]}"; do
  name="${pair%%:*}"; id="${pair##*:}"
  echo "fetching $name ($id) from $START_TIME..."
  curl -sS "https://ssd.jpl.nasa.gov/api/horizons.api?format=text&COMMAND=%27${id}%3B%27&OBJ_DATA=%27NO%27&MAKE_EPHEM=%27YES%27&EPHEM_TYPE=%27ELEMENTS%27&CENTER=%27500@10%27&START_TIME=%27${START_TIME}%27&STOP_TIME=%272100-01-02%27&STEP_SIZE=%27180d%27&REF_PLANE=%27ECLIPTIC%27&REF_SYSTEM=%27J2000%27&OUT_UNITS=%27AU-D%27" -o "/tmp/hz_${name}.txt"
done
node -e '
const fs=require("fs");
